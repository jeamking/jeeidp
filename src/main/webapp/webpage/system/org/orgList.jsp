	  ...
	  <tbody>
	  	<c:forEach items="${orgList }" var="org">
	  		...
	  		>
		  		...
		  		<td>${org.orgCode}</td>
		  		<td>
		  			<a href="javascript:;" class="btn btn-sm blue" onclick="update('/org/org?id=${org.id}')">
		  				<i class="fa fa-edit"></i>编辑
		  			</a>
						<a href="javascript:;" class="btn btn-sm red" onclick="del('/org/delete?id=${org.id}')">
					...
					success: function(data){
						...
						for(var i = 0; i < data.length; i++){
							...
							rows += '<tr id="'+data[i].id+'" pId="'+data[i].parentId+'" '+hasChild+'>\n'
								...
								+	'<td>\n'
								+	'<a href="javascript:;" class="btn btn-sm blue" onclick="update(\'/org/org?id='+data[i].id+'\')">'
								+	'<i class="fa fa-edit"></i>编辑</a>\n'
								+	'<a href="javascript:;" class="btn btn-sm red" onclick="del(\'/org/delete?id='+data[i].id+'\')">'
